# This script ensures the CSS file is visible in the Shirasagi backend folder view
# It creates the proper folder structure and file registration for Shirasagi

puts "Starting CSS file visibility fix for folder view..."
...
# Create a node for CSS files
css_node = Cms::Node.find_or_create_by(site_id: site.id, filename: 'css') do |n|
  n.name = 'CSS'
  n.route = 'cms/node'
  n.state = 'public'
...
existing_file = SS::File.where(site_id: site.id, filename: 'css/kanamic.css').first
...
# Create a page for the CSS file to make it visible in the folder view
css_page = Cms::Page.find_or_create_by(site_id: site.id, filename: 'css/kanamic') do |p|
  p.name = 'kanamic.css'
  p.layout_id = nil
  p.state = 'public'
  p.group_ids = [group.id]
  p.file_ids = [file.id]
end
puts "Created CSS page: #{css_page.name} (#{css_page.id})"

puts "CSS file visibility fix for folder view completed successfully!"
