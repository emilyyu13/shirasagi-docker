# This script creates a CSS folder that is visible in the Shirasagi backend folder view
# It ensures the CSS file appears when the "フォルダー" link is clicked

puts "Starting creation of visible CSS folder..."
...
existing_nodes = Cms::Node.where(site_id: site.id, filename: /^css/)
existing_nodes.each do |node|
  puts "Removing existing node: #{node.name} (#{node.id})"
  node.destroy
end

# Create a visible folder node
folder_node = Cms::Node.new
folder_node.site_id = site.id
folder_node.name = "CSS"
folder_node.filename = "css"
folder_node.route = "cms/node"
folder_node.state = "public"
folder_node.group_ids = [group.id]
folder_node.save!
...
puts "Creation of visible CSS folder completed successfully!"
