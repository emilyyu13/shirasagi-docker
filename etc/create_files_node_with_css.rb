# This script creates a file node with ASCII filename "files" but Japanese display name "ファイル"
# It ensures the CSS file appears when the "フォルダー" link is clicked

puts "Starting creation of files node with CSS..."
...
# Create a file node at the root level with ASCII filename "files" but Japanese display name "ファイル"
file_node = Cms::Node.new
file_node.site_id = site.id
file_node.name = "ファイル"
file_node.filename = "files"
...
existing_files = SS::File.where(site_id: site.id, filename: /^files\//)
...
file.filename = "files/kanamic.css"
...
puts "Creation of files node with CSS completed successfully!"
