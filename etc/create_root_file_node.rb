# This script creates a file node at the root level that is visible in the Shirasagi backend folder view
# It ensures the CSS file appears when the "フォルダー" link is clicked

puts "Starting creation of root file node..."
...
existing_nodes = Cms::Node.where(site_id: site.id, filename: /^(css|files)/)
existing_nodes.each do |node|
  puts "Removing existing node: #{node.name} (#{node.id})"
  node.destroy
end

# Create a file node at the root level
file_node = Cms::Node.new
file_node.site_id = site.id
file_node.name = "ファイル"
file_node.filename = "file"
file_node.route = "uploader/file"
...
file_dir = "#{site.path}/file"
Fs.mkdir_p(file_dir) unless Fs.exist?(file_dir)
puts "Created file directory: #{file_dir}"

# Create CSS directory under the file directory
css_dir = "#{file_dir}/css"
...
existing_files = SS::File.where(site_id: site.id, filename: /^file\/css\//)
...
file.filename = "file/css/kanamic.css"
...
puts "Creation of root file node completed successfully!"
